                               ...
                               unsigned *outputs_mask)
{
    ...
    if (dp->transcoder->global_param->debug_ts) {
                               ...
                               AVFrame *frame)
{
    ...
    if (ret < 0) {
        ...
        return dp->transcoder->global_param->exit_on_error ? ret : 0;
    }
...
static int packet_decode(DecoderPriv *dp, AVPacket *pkt, AVFrame *frame)
{
    ...
    if (ret < 0 && !(ret == AVERROR_EOF && !pkt)) {
        ...
        dp->dec.decode_errors++;
        if (dp->transcoder->global_param->exit_on_error)
            return ret;
    }
    ...
    while (1) {
        ...
        if (frame->decode_error_flags || (frame->flags & AV_FRAME_FLAG_CORRUPT)) {
            av_log(dp, dp->transcoder->global_param->exit_on_error ? AV_LOG_FATAL : AV_LOG_WARNING,
...
static int decoder_thread(void *arg)
{
    ...
    if (ret >= 0) {
        ...
        if (err_rate > dp->transcoder->global_param->max_error_rate) {
            av_log(dp, AV_LOG_FATAL, "Decode error rate %g exceeds maximum %g\n",
                   err_rate, dp->transcoder->global_param->max_error_rate);
    ...
    for (int i = 0; i < dp->nb_views_requested; i++) {
        ...
        case VIEW_SPECIFIER_TYPE_IDX:
            if (vs->val >= nb_view_ids_av) {
                av_log(dp, dp->transcoder->global_param->exit_on_error ? AV_LOG_ERROR : AV_LOG_WARNING,
...
static HWDevice *hw_device_match_by_codec(const AVCodec *codec, FFmpegTranscoder *global_param)
{
                                      ...
                                      const char *hwaccel_device)
{
    ...
    if (hwaccel_device) {
        dev = hw_device_get_by_name(hwaccel_device, dp->transcoder->global_param);
        if (!dev) {
...
int dec_create(const OptionsContext *o, const char *arg, Scheduler *sch)
{
    ...
    if (ret < 0)
        return ret;
    dp->transcoder->global_param = o->transcoder->global_param;
    dp->index = o->transcoder->global_param->nb_decoders;

    ret = GROW_ARRAY(o->transcoder->global_param->decoders, o->transcoder->global_param->nb_decoders);
    if (ret < 0) {
        dec_free((Decoder **)&dp);
        return ret;
    }

    o->transcoder->global_param->decoders[o->transcoder->global_param->nb_decoders - 1] = (Decoder *)dp;

    of_index = strtol(arg, &p, 0);
    if (of_index < 0 || of_index >= o->transcoder->global_param->nb_output_files) {
        av_log(dp, AV_LOG_ERROR, "Invalid output file index '%d' in %s\n", of_index, arg);
        return AVERROR(EINVAL);
    }
    of = o->transcoder->global_param->output_files[of_index];
