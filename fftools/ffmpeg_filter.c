                       ...
                       uint8_t **data, int *len, FFmpegTranscoder *transcoder)
{
    ...
    *len  = 0;
    ...
    ret = avio_open2(&io, path, AVIO_FLAG_READ, &global_param->int_cb, NULL);
    if (ret < 0) {
                            ...
                            const char *key, const char *val, FFmpegTranscoder *transcoder)
{
    ...
    if (o->type == AV_OPT_TYPE_BINARY) {
        ...
        ret = read_binary(logctx, val, &data, &len,transcoder);
        if (ret < 0)
            goto err_load;
    ...
    for (size_t i = 0; i < seg->nb_chains; i++) {
        ...
        for (size_t j = 0; j < ch->nb_filters; j++) {
            ...
            while ((e = av_dict_iterate(p->opts, e))) {
                int ret = filter_opt_apply(logctx, p->filter, e->key, e->value,transcoder);
                if (ret < 0)
                    return ret;
            }
                       ...
                       AVBufferRef *hw_device, FFmpegTranscoder *transcoder)
{
    ...
    *outputs = NULL;
    ...
    ret = graph_opts_apply(logctx, seg,transcoder);
    if (ret < 0)
        goto fail;
              ...
              const OutputFilterOptions *opts, FFmpegTranscoder *transcoder)
{
    ...
    if (pfg) {
        *pfg = fg;
        ...
        ret = av_dynarray_add_nofree(&global_param->filtergraphs, &global_param->nb_filtergraphs, fgp);
        if (ret < 0) {
            av_freep(graph_desc);
            av_freep(&fgp);
            return ret;
        ...
        fg->index = global_param->nb_filtergraphs - 1;
    }

    fg->class       = &fg_class;
    fg->graph_desc  = *graph_desc;
    fgp->disable_conversions = !global_param->auto_conversion_filters;
    fgp->nb_threads          = -1;
    fgp->sch                 = sch;
    ...
    *graph_desc = NULL;
    ...
    ret = graph_parse(fg, graph, fg->graph_desc, &inputs, &outputs,
                      hw_device_for_filter(transcoder),transcoder);
    if (ret < 0)
        goto fail;
                     ...
                     const OutputFilterOptions *opts, FFmpegTranscoder *transcoder)
{
    ...
    ret = fg_create(pfg, graph_desc, sch, NULL, transcoder);
    if (ret < 0)
        return ret;
    fg  = *pfg;
    fgp = fgp_from_fg(fg);
...
static int fg_complex_bind_input(FilterGraph *fg, InputFilter *ifilter, int commit, FFmpegTranscoder *transcoder)
{
    ...
    if (ifilter->linklabel && !strncmp(ifilter->linklabel, "dec:", 4)) {
        ...
        if (dec_idx < 0 || dec_idx >= global_param->nb_decoders) {
        ...
        ret = ifilter_bind_dec(ifp, global_param->decoders[dec_idx], &vs);
        if (ret < 0)
        ...
        for (int i = 0; i < global_param->nb_filtergraphs; i++) {
            FilterGraph *fg_src = global_param->filtergraphs[i];
        ...
        if (file_idx < 0 || file_idx >= global_param->nb_input_files) {
            av_log(fg, AV_LOG_FATAL, "Invalid file index %d in filtergraph description %s.\n",
                   file_idx, fg->graph_desc);
            return AVERROR(EINVAL);
        }
        s = global_param->input_files[file_idx]->ctx;
        ...
        ist = global_param->input_files[file_idx]->streams[st->index];
        ...
        ist = ist_find_unused(type, transcoder);
        if (!ist) {
...
static int bind_inputs(FilterGraph *fg, int commit, FFmpegTranscoder *transcoder)
{
    ...
    for (int i = 0; i < fg->nb_inputs; i++) {
        ...
        ret = fg_complex_bind_input(fg, &ifp->ifilter, commit, transcoder);
        if (ret < 0)
            return ret;
    }
...
int fg_finalise_bindings(FFmpegTranscoder *transcoder)
{
    ...
    FFmpegGlobalParam *global_param = transcoder->global_param;
    for (int i = 0; i < global_param->nb_filtergraphs; i++) {
        ret = bind_inputs(global_param->filtergraphs[i], 0, transcoder);
        if (ret < 0)
            return ret;
    }

    // check that all outputs were bound
    for (int i = global_param->nb_filtergraphs - 1; i >= 0; i--) {
        FilterGraph *fg = global_param->filtergraphs[i];
        FilterGraphPriv *fgp = fgp_from_fg(global_param->filtergraphs[i]);
        ...
        for (int j = 0; j < fg->nb_outputs; j++) {
            ...
            if (!ofp_from_ofilter(output)->needed) {
                ...
                fg_free(&global_param->filtergraphs[i]);
                global_param->nb_filtergraphs--;
                if (global_param->nb_filtergraphs > 0)
                    memmove(&global_param->filtergraphs[i],
                            &global_param->filtergraphs[i + 1],
                            (global_param->nb_filtergraphs - i) * sizeof(*global_param->filtergraphs));
                break;
            }
        }
    }
    ...
    for (int i = 0; i < global_param->nb_filtergraphs; i++) {
        ret = bind_inputs(global_param->filtergraphs[i], 1, transcoder);
        if (ret < 0)
            return ret;
    }
...
static int configure_filtergraph(FilterGraph *fg, FilterGraphThread *fgt, FFmpegTranscoder *transcoder)
{
    ...
    if (simple) {
        ...
        if (global_param->filter_nbthreads) {
            ret = av_opt_set(fgt->graph, "threads", global_param->filter_nbthreads, 0);
            if (ret < 0)
                goto fail;
        ...
        fgt->graph->nb_threads = global_param->filter_complex_nbthreads;
    }

    if (global_param->filter_buffered_frames) {
        ret = av_opt_set_int(fgt->graph, "max_buffered_frames", global_param->filter_buffered_frames, 0);
        if (ret < 0)
            return ret;
    }

    hw_device = hw_device_for_filter(transcoder);

    ret = graph_parse(fg, fgt->graph, graph_desc, &inputs, &outputs, hw_device, transcoder);
    if (ret < 0)
        goto fail;
                               ...
                               int64_t *nb_frames, int64_t *nb_frames_prev, FFmpegTranscoder *transcoder)
{
    ...
    sync_ipts = adjust_frame_pts_to_encoder_tb(ofilter->graph, frame,
                                               ofp->tb_out, ofp->ts_offset,transcoder);
    ...
    *nb_frames = 1;
    ...
    case VSYNC_CFR:
        ...
        if (global_param->frame_drop_threshold && delta < global_param->frame_drop_threshold && fps->frame_number) {
            *nb_frames = 0;
        } else if (delta < -1.1)
            *nb_frames = 0;
        else if (delta > 1.1) {
...
finish:
    ...
    if (*nb_frames > (*nb_frames_prev && fps->last_dropped) + (*nb_frames > *nb_frames_prev)) {
        ...
        if (*nb_frames > global_param->dts_error_threshold * 30) {
                           ...
                           AVFrame *frame, FFmpegTranscoder *transcoder)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO && (frame || fgt->got_frame))
        video_sync_process(ofp, frame, &nb_frames, &nb_frames_prev, transcoder);
                          ...
                          AVFrame *frame, FFmpegTranscoder *transcoder)
{
    ...
    if (ret == AVERROR_EOF && !fgt->eof_out[ofp->ofilter.index]) {
        ret = fg_output_frame(ofp, fgt, NULL, transcoder);
        return (ret < 0) ? ret : 1;
    ...
    ret = fg_output_frame(ofp, fgt, frame,transcoder);
    av_frame_unref(frame);
    if (ret < 0)
        return ret;
                       ...
                       AVFrame *frame, FFmpegTranscoder *transcoder)
{
    ...
    while (fgp->nb_outputs_done < fg->nb_outputs) {
        ...
        for (int i = 0; i < fg->nb_outputs; i++) {
            ...
            ret = 0;
            while (!ret) {
                ret = fg_output_step(ofp, fgt, frame, transcoder);
                if (ret < 0)
                    return ret;
            }
        }
    ...
    if (ifilter->filter) {
        ...
        if (ifp->format < 0) {
            ...
            if (ifilter_has_all_input_formats(ifilter->graph)) {
                ret = configure_filtergraph(ifilter->graph, fgt, transcoder);
                if (ret < 0) {
                      ...
                      InputFilter *ifilter, AVFrame *frame, FFmpegTranscoder *transcoder)
{
    ...
    if (need_reinit || !fgt->graph) {
        ...
        ret = fgt->graph ? read_frames(fg, fgt, tmp, transcoder) : 0;
        av_frame_free(&tmp);
        if (ret < 0)
            return ret;
        ...
        ret = configure_filtergraph(fg, fgt, transcoder);
        if (ret < 0) {
...
static int filter_thread(void *arg)
{
    ...
    if (ifilter_has_all_input_formats(fg)) {
        ret = configure_filtergraph(fg, &fgt, fgp->transcoder);
        if (ret < 0) {
    ...
    while (1) {
        ...
        if (ifp->type_src == AVMEDIA_TYPE_SUBTITLE) {
            ...
            ret = send_frame(fg, &fgt, ifilter, fgt.frame, fgp->transcoder);
        } else {
            av_assert1(o == FRAME_OPAQUE_EOF);
            ret = send_eof(&fgt, ifilter, fgt.frame->pts, fgt.frame->time_base, fgp->transcoder);
        }
        av_frame_unref(fgt.frame);
        if (ret == AVERROR_EOF) {
...
read_frames:
        ...
        ret = read_frames(fg, &fgt, fgt.frame, fgp->transcoder);
        if (ret == AVERROR_EOF) {
    ...
    for (unsigned i = 0; i < fg->nb_outputs; i++) {
        ...
        ret = fg_output_frame(ofp, &fgt, NULL, fgp->transcoder);
        if (ret < 0)
            goto finish;
    }
...
finish:
    ...
    if (fgp->transcoder->global_param->print_graphs || fgp->transcoder->global_param->print_graphs_file)
        print_filtergraph(fg, fgt.graph,fgp->transcoder);
