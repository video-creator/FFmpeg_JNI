...
static void ffmpeg_cleanup(int ret, FFGlobalParam *g)
{
    ...
    for (int i = 0; i < g->nb_filtergraphs; i++)
        fg_free(&g->filtergraphs[i]);
    av_freep(&g->filtergraphs);

    for (int i = 0; i < g->nb_output_files; i++)
        of_free(&g->output_files[i]);

    for (int i = 0; i < g->nb_input_files; i++)
        ifile_close(&g->input_files[i]);

    for (int i = 0; i < g->nb_decoders; i++)
        dec_free(&g->decoders[i]);
    av_freep(&g->decoders);
    ...
    av_freep(&g->print_graphs_file);
    av_freep(&g->print_graphs_format);
...
OutputStream *ost_iter(OutputStream *prev,FFGlobalParam *g)
{
    ...
    for (; of_idx < g->nb_output_files; of_idx++) {
        OutputFile *of = g->output_files[of_idx];
...
InputStream *ist_iter(InputStream *prev,FFGlobalParam *g)
{
    ...
    for (; if_idx < g->nb_input_files; if_idx++) {
        InputFile *f = g->input_files[if_idx];
    ...
    if (g->do_benchmark_all) {
        ...
        if (fmt) {
            ...
            av_log(NULL, AV_LOG_INFO,
                   ...
                   t.user_usec - g->current_time.user_usec,
                   t.sys_usec - g->current_time.sys_usec,
                   t.real_usec - g->current_time.real_usec, buf);
        }
        g->current_time = t;
    }
}
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts, FFGlobalParam *g)
{
    ...
    if (!g->print_stats && !is_last_report && !g->progress_avio)
        return;
    ...
    if (!is_last_report) {
        ...
        if (((cur_time - last_time) < g->stats_period && !first_report) ||
            (first_report && atomic_load(&g->nb_output_dumped) < g->nb_output_files))
            return;
        last_time = cur_time;
    }
    ...
    if (g->print_stats || is_last_report) {
        const char end = is_last_report ? '\n' : '\r';
        if (g->print_stats==1 && AV_LOG_INFO > av_log_get_level()) {
    ...
    if (g->progress_avio) {
        ...
        avio_write(g->progress_avio, buf_script.str,
                   FFMIN(buf_script.len, buf_script.size - 1));
        avio_flush(g->progress_avio);
        av_bprint_finalize(&buf_script, NULL);
        if (is_last_report) {
            if ((ret = avio_closep(&g->progress_avio)) < 0)
                av_log(NULL, AV_LOG_ERROR,
...
static int check_keyboard_interaction(int64_t cur_time,FFGlobalParam *g)
{
    ...
    if (key == 'c' || key == 'C'){
        ...
        if (k > 0 &&
            ...
            for (i = 0; i < g->nb_filtergraphs; i++)
                fg_send_command(g->filtergraphs[i], time, target, command, arg,
                                key == 'C');
        } else {
            av_log(NULL, AV_LOG_ERROR,
...
static void init_global_param_val(FFGlobalParam *g) {
    ...
    g->frame_drop_threshold = 0;
    g->do_benchmark = 0;
    g->do_benchmark_all = 0;
    g->do_hex_dump = 0;
    g->do_pkt_dump = 0;
    g->copy_ts = 0;
    g->start_at_zero = 0;
    ...
    g->abort_on_flags = 0;
    g->print_stats = -1;
    g->stdin_interaction = 1;
    g->max_error_rate = 2.0 / 3;

    g->filter_complex_nbthreads = 0;
    g->filter_buffered_frames = 0;
    g->vstats_version = 2;
    g->print_graphs = 0;
    g->print_graphs_file = NULL;
    g->print_graphs_format = NULL;
    g->auto_conversion_filters = 1;
    g->stats_period = 500000;

    g->file_overwrite = 0;
    g->no_file_overwrite = 0;
    g->ignore_unknown_streams = 0;
