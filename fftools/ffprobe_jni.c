...
typedef struct FFProbeGlobalParam {
    ...
    char* show_data_hash;
    char* data_dump_format;

    ReadInterval* read_intervals;
    int read_intervals_nb;

    int find_stream_info;
    EntrySelection selected_entries[FF_ARRAY_ELEMS(sections)];
...
//     new_log_buffer = av_realloc_array(gp->log_buffer, gp->log_buffer_size + 1, sizeof(*gp->log_buffer));
//     if (new_log_buffer) {
//         char *msg;
//         int i;

//         gp->log_buffer = new_log_buffer;
//         memset(&gp->log_buffer[gp->log_buffer_size], 0, sizeof(gp->log_buffer[gp->log_buffer_size]));
//         gp->log_buffer[gp->log_buffer_size].context_name= avc ? av_strdup(avc->item_name(ptr)) : NULL;
//         if (avc) {
//             if (avc->get_category) gp->log_buffer[gp->log_buffer_size].category = avc->get_category(ptr);
//             else                   gp->log_buffer[gp->log_buffer_size].category = avc->category;
//         }
//         gp->log_buffer[gp->log_buffer_size].log_level   = level;
//         msg = gp->log_buffer[gp->log_buffer_size].log_message = av_strdup(line);
...
//             if (parent && *parent) {
//                 gp->log_buffer[gp->log_buffer_size].parent_name = av_strdup((*parent)->item_name(parent));
//                 gp->log_buffer[gp->log_buffer_size].parent_category =
...
static void clear_log(FFProbeGlobalParam *gp, int need_lock)
{
    ...
    if (need_lock)
        ff_mutex_lock(&gp->log_mutex);
    for (i=0; i<gp->log_buffer_size; i++) {
        av_freep(&gp->log_buffer[i].context_name);
        av_freep(&gp->log_buffer[i].parent_name);
        av_freep(&gp->log_buffer[i].log_message);
    }
    gp->log_buffer_size = 0;
    if(need_lock)
        ff_mutex_unlock(&gp->log_mutex);
}
...
static int show_log(AVTextFormatContext *tfc, int section_ids, int section_id, int log_level)
{
    ...
    for (i=0; i<gp->log_buffer_size; i++) {
        if (gp->log_buffer[i].log_level <= log_level) {
            avtext_print_section_header(tfc, NULL, section_id);
            print_str("context", gp->log_buffer[i].context_name);
            print_int("level", gp->log_buffer[i].log_level);
            print_int("category", gp->log_buffer[i].category);
            if (gp->log_buffer[i].parent_name) {
                print_str("parent_context", gp->log_buffer[i].parent_name);
                print_int("parent_category", gp->log_buffer[i].parent_category);
            } else {
                print_str_opt("parent_context", "N/A");
                print_str_opt("parent_category", "N/A");
            }
            print_str("message", gp->log_buffer[i].log_message);
            avtext_print_section_footer(tfc);
        }
    }
    clear_log(gp,0);
...
static void show_packet(AVTextFormatContext *tfc, InputFile *ifile, AVPacket *pkt, int packet_idx)
{
    ...
    if (pkt->side_data_elems) {
        ...
        if (side_metadata && size && gp->do_show_packet_tags) {
                       ...
                       AVFormatContext *fmt_ctx)
{
    ...
    if (gp->do_show_frame_tags)
        show_tags(tfc, frame->metadata, SECTION_ID_FRAME_TAGS);
    if (gp->do_show_log)
        show_log(tfc, SECTION_ID_FRAME_LOGS, SECTION_ID_FRAME_LOG, gp->do_show_log);
                                          ...
                                          int *packet_new)
{
    ...
    if (got_frame) {
        ...
        gp->nb_streams_frames[pkt->stream_index]++;
        if (gp->do_show_frames)
            if (is_sub)
        ...
        if (!is_sub && gp->do_analyze_frames) {
            for (int i = 0; i < frame->nb_side_data; i++) {
                if (frame->side_data[i]->type == AV_FRAME_DATA_A53_CC)
                    gp->streams_with_closed_captions[pkt->stream_index] = 1;
                else if (frame->side_data[i]->type == AV_FRAME_DATA_FILM_GRAIN_PARAMS)
                    gp->streams_with_film_grain[pkt->stream_index] = 1;
            }
        }
                                 ...
                                 const ReadInterval *interval, int64_t *cur_ts)
{
    ...
    while (!av_read_frame(fmt_ctx, pkt)) {
        if (fmt_ctx->nb_streams > gp->nb_streams) {
            REALLOCZ_ARRAY_STREAM(gp->nb_streams_frames,  gp->nb_streams, fmt_ctx->nb_streams);
            REALLOCZ_ARRAY_STREAM(gp->nb_streams_packets, gp->nb_streams, fmt_ctx->nb_streams);
            REALLOCZ_ARRAY_STREAM(gp->selected_streams,   gp->nb_streams, fmt_ctx->nb_streams);
            REALLOCZ_ARRAY_STREAM(gp->streams_with_closed_captions,   gp->nb_streams, fmt_ctx->nb_streams);
            REALLOCZ_ARRAY_STREAM(gp->streams_with_film_grain,        gp->nb_streams, fmt_ctx->nb_streams);
            gp->nb_streams = fmt_ctx->nb_streams;
        }
        if (gp->selected_streams[pkt->stream_index]) {
            ...
            frame_count++;
            if (gp->do_read_packets) {
                if (gp->do_show_packets)
                    show_packet(tfc, ifile, pkt, i++);
                gp->nb_streams_packets[pkt->stream_index]++;
            }
            if (gp->do_read_frames) {
                int packet_new = 1;
                FrameData *fd;
...
static int read_packets( AVTextFormatContext *tfc, InputFile *ifile)
{
    ...
    if (gp->read_intervals_nb == 0) {
        ReadInterval interval = (ReadInterval) { .has_start = 0, .has_end = 0 };
        ret = read_interval_packets(tfc, ifile, &interval, &cur_ts);
    } else {
        for (i = 0; i < gp->read_intervals_nb; i++) {
            ret = read_interval_packets(tfc, ifile, &gp->read_intervals[i], &cur_ts);
            if (ret < 0)
                break;
        }
    }
...
static int show_stream(AVTextFormatContext *tfc, AVFormatContext *fmt_ctx, int stream_idx, InputStream *ist, int container)
{
    ...
    if (!gp->do_bitexact && (profile = avcodec_profile_name(par->codec_id, par->profile)))
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (dec_ctx) {
            ...
            if (gp->do_analyze_frames) {
                print_int("closed_captions", gp->streams_with_closed_captions[stream->index]);
                print_int("film_grain",      gp->streams_with_film_grain[stream->index]);
            }
        }
    ...
    if (gp->nb_streams_frames[stream_idx])  print_fmt    ("nb_read_frames", "%"PRIu64, gp->nb_streams_frames[stream_idx]);
    else                                print_str_opt("nb_read_frames", "N/A");
    if (gp->nb_streams_packets[stream_idx]) print_fmt    ("nb_read_packets", "%"PRIu64, gp->nb_streams_packets[stream_idx]);
    ...
    if (gp->do_show_stream_disposition) {
...
static int show_program(AVTextFormatContext *tfc, InputFile *ifile, AVProgram *program)
{
    ...
    for (i = 0; i < program->nb_stream_indexes; i++) {
        if (gp->selected_streams[program->stream_index[i]]) {
...
static int show_stream_group(AVTextFormatContext *tfc, InputFile *ifile, AVStreamGroup *stg)
{
    ...
    for (i = 0; i < stg->nb_streams; i++) {
        if (gp->selected_streams[stg->streams[i]->index]) {
...
#define GET_DECODER(param,type_) \
    ret = get_decoder_by_name(param->type_##_codec_name, &fmt_ctx->type_##_codec); \
    if (ret < 0) return ret;

    GET_DECODER(gp,audio);
    GET_DECODER(gp,data);
    GET_DECODER(gp,subtitle);
    GET_DECODER(gp,video);
    return 0;
}
                           ...
                           const char *print_filename)
{
    ...
    err = set_decoders(gp,fmt_ctx);
    if (err < 0)
        return err;
    if (!av_dict_get(gp->format_opts, "scan_all_pmts", NULL, AV_DICT_MATCH_CASE)) {
        av_dict_set(&gp->format_opts, "scan_all_pmts", "1", AV_DICT_DONT_OVERWRITE);
        scan_all_pmts_set = 1;
    }
    if ((err = avformat_open_input(&fmt_ctx, filename,
                                   gp->iformat, &gp->format_opts)) < 0) {
    ...
    if (scan_all_pmts_set)
        av_dict_set(&gp->format_opts, "scan_all_pmts", NULL, AV_DICT_MATCH_CASE);
    while ((t = av_dict_iterate(gp->format_opts, t)))
        av_log(NULL, AV_LOG_WARNING, "Option %s skipped - not known to demuxer.\n", t->key);

    if (gp->find_stream_info) {
        AVDictionary **opts;
        int orig_nb_streams = fmt_ctx->nb_streams;

        err = setup_find_stream_info_opts(fmt_ctx, gp->codec_opts, &opts);
        if (err < 0)
            return err;
    ...
    for (i = 0; i < fmt_ctx->nb_streams; i++) {
...

        {
            ...
            err = filter_codec_opts(gp->codec_opts, stream->codecpar->codec_id,
            ...
            if (gp->do_show_log) {
                ...
                av_dict_set(&gp->codec_opts, "threads", "1", 0);
            }
                      ...
                      const char *print_filename)
{
    ...
    gp->do_analyze_frames = gp->do_analyze_frames && gp->do_show_streams;
    gp->do_read_frames = gp->do_show_frames || gp->do_count_frames || gp->do_analyze_frames;
    gp->do_read_packets = gp->do_show_packets || gp->do_count_packets;

    ret = open_input_file(gp, &ifile, filename, print_filename);
    if (ret < 0)
        goto end;

#define CHECK_END if (ret < 0) goto end

    gp->nb_streams = ifile.fmt_ctx->nb_streams;
    REALLOCZ_ARRAY_STREAM(gp->nb_streams_frames,0,ifile.fmt_ctx->nb_streams);
    REALLOCZ_ARRAY_STREAM(gp->nb_streams_packets,0,ifile.fmt_ctx->nb_streams);
    REALLOCZ_ARRAY_STREAM(gp->selected_streams,0,ifile.fmt_ctx->nb_streams);
    REALLOCZ_ARRAY_STREAM(gp->streams_with_closed_captions,0,ifile.fmt_ctx->nb_streams);
    REALLOCZ_ARRAY_STREAM(gp->streams_with_film_grain,0,ifile.fmt_ctx->nb_streams);

    for (i = 0; i < ifile.fmt_ctx->nb_streams; i++) {
        if (gp->stream_specifier) {
            ret = avformat_match_stream_specifier(ifile.fmt_ctx,
                                                  ifile.fmt_ctx->streams[i],
                                                  gp->stream_specifier);
            CHECK_END;
            else
                gp->selected_streams[i] = ret;
            ret = 0;
        } else {
            gp->selected_streams[i] = 1;
        }
        if (!gp->selected_streams[i])
            ifile.fmt_ctx->streams[i]->discard = AVDISCARD_ALL;
    }

    if (gp->do_read_frames || gp->do_read_packets) {
        if (gp->do_show_frames && gp->do_show_packets &&
            tfc->formatter->flags & AV_TEXTFORMAT_FLAG_SUPPORTS_MIXED_ARRAY_CONTENT)
            section_id = SECTION_ID_PACKETS_AND_FRAMES;
        else if (gp->do_show_packets && !gp->do_show_frames)
    ...
    if (gp->do_show_programs) {
        ret = show_programs(tfc, &ifile);
        CHECK_END;
    }

    if (gp->do_show_stream_groups) {
        ret = show_stream_groups(tfc, &ifile);
        CHECK_END;
    }

    if (gp->do_show_streams) {
        ret = show_streams(tfc, &ifile);
        CHECK_END;
    }
    if (gp->do_show_chapters) {
        ret = show_chapters(tfc, &ifile);
        CHECK_END;
    }
    if (gp->do_show_format) {
    ...
    av_freep(&gp->nb_streams_frames);
    av_freep(&gp->nb_streams_packets);
    av_freep(&gp->selected_streams);
    av_freep(&gp->streams_with_closed_captions);
    av_freep(&gp->streams_with_film_grain);
    av_freep(&gp->audio_codec_name);
    av_freep(&gp->data_codec_name);
...
static void ffprobe_show_pixel_formats(AVTextFormatContext *tfc)
{
    ...
    while (pixdesc = av_pix_fmt_desc_next(pixdesc)) {
        ...
        if (gp->do_show_pixel_format_flags) {
        ...
        if (gp->do_show_pixel_format_components && (pixdesc->nb_components > 0)) {
                                             ...
                                             int show_all_entries, AVDictionary *entries)
{
    ...
    if (show_all_entries) {
        ...
        for (const int *id = section->children_ids; *id != -1; id++)
            mark_section_show_entries(gp, *id, show_all_entries, entries);
    } else {
                         ...
                         int show_all_entries, AVDictionary *entries)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(sections); i++) {
        ...
        if (!strcmp(section_name, section->name) ||
            ...
            ret++;
            mark_section_show_entries(gp, section->id, show_all_entries, entries);
        }
    }
    return ret;
}
...
static int opt_show_entries(void *optctx, const char *opt, const char *arg)
{
    ...
    while (*p) {
        ...
        ret = match_section(gp,section_name, show_all_entries, entries);
        if (ret == 0) {
...
static int opt_input_file(void *optctx, const char *arg)
{
    ...
    if (gp->input_filename) {
        av_log(NULL, AV_LOG_ERROR,
                ...
                arg, gp->input_filename);
        return AVERROR(EINVAL);
    }
    if (!strcmp(arg, "-"))
        arg = "fd:";
    gp->input_filename = av_strdup(arg);
    if (!gp->input_filename)
        return AVERROR(ENOMEM);
...
static int opt_output_file_o(void *optctx, const char *opt, const char *arg)
{
    ...
    if (gp->output_filename) {
        av_log(NULL, AV_LOG_ERROR,
                ...
                arg, gp->output_filename);
        return AVERROR(EINVAL);
    }
    if (!strcmp(arg, "-"))
        arg = "fd:";
    gp->output_filename = av_strdup(arg);
    if (!gp->output_filename)
        return AVERROR(ENOMEM);
...
void show_help_default(const char *opt, const char *arg)
{
    ...
    // show_usage();
    // show_help_options(real_options, "Main options:", 0, 0);
    // printf("\n");
...
static int parse_read_intervals(FFProbeGlobalParam *gp, const char *intervals_spec)
{
    ...
    gp->read_intervals = av_malloc_array(n, sizeof(*gp->read_intervals));
    if (!gp->read_intervals) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
    gp->read_intervals_nb = n;
    ...
    for (i = 0; p; i++) {
        ...
        av_assert0(i < gp->read_intervals_nb);
        next = strchr(p, ',');
        if (next)
            *next++ = 0;

        gp->read_intervals[i].id = i;
        ret = parse_read_interval(p, &gp->read_intervals[i]);
        if (ret < 0) {
        ...
        log_read_interval(&gp->read_intervals[i], NULL, AV_LOG_VERBOSE);
        p = next;
    }
    av_assert0(i == gp->read_intervals_nb);
...
static int opt_pretty(void *optctx, const char *opt, const char *arg)
{
    ...
    gp->use_value_sexagesimal_format = 1;
    return 0;
}
...
static int opt_codec(void *optctx, const char *opt, const char *arg)
{
   ...
   switch (spec[0]) {
   case 'a' : name = &gp->audio_codec_name;    break;
   case 'd' : name = &gp->data_codec_name;     break;
   case 's' : name = &gp->subtitle_codec_name; break;
   case 'v' : name = &gp->video_codec_name;    break;
   default:
       av_log(NULL, AV_LOG_ERROR,
...
#define DEFINE_OPT_SHOW_SECTION(section, target_section_id)             \
    ...
    {                                                                   \
        mark_section_show_entries(optctx, SECTION_ID_##target_section_id, 1, NULL); \
        return 0;                                                       \
    }
...
static const OptionDef real_options[] = {
    ...
    { "unit",                  OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, { .off       = OFFSET(show_value_unit)}, "show unit of the displayed values" },
    { "prefix",                OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, { .off       = OFFSET(use_value_prefix)}, "use SI prefixes for the displayed values" },
    { "byte_binary_prefix",    OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, { .off       = OFFSET(use_byte_value_binary_prefix)},
      "use binary prefixes for byte units" },
    { "sexagesimal",           OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, { .off       = OFFSET(use_value_sexagesimal_format)},
      "use sexagesimal format HOURS:MM:SS.MICROSECONDS for time units" },
    { "pretty",                OPT_TYPE_FUNC,        0, {.func_arg = opt_pretty},
      "prettify the format of displayed values, make it more human readable" },
    { "output_format",         OPT_TYPE_STRING,      OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(output_format) },
      "set the output printing format (available formats are: default, compact, csv, flat, ini, json, xml)", "format" },
    { "print_format",          OPT_TYPE_STRING,      OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(output_format) }, "alias for -output_format (deprecated)" },
    { "of",                    OPT_TYPE_STRING,      OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(output_format) }, "alias for -output_format", "format" },
    { "select_streams",        OPT_TYPE_STRING,      OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(stream_specifier) }, "select the specified streams", "stream_specifier" },
    { "sections",              OPT_TYPE_FUNC, OPT_EXIT, {.func_arg = opt_sections}, "print sections structure and section information, and exit" },
    { "data_dump_format",      OPT_TYPE_STRING,      OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(data_dump_format) }, "set data dump format (available formats are: xxd, base64)" },
    { "show_data",             OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(do_show_data) }, "show packets data" },
    { "show_data_hash",        OPT_TYPE_STRING,      OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(show_data_hash) }, "show packets data hash" },
...
#if HAVE_THREADS
    { "show_log",              OPT_TYPE_INT,         OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(do_show_log) }, "show log" },
#endif
    ...
    { "count_frames",          OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(do_count_frames) }, "count the number of frames per stream" },
    { "count_packets",         OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(do_count_packets) }, "count the number of packets per stream" },
    ...
    { "show_private_data",     OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(show_private_data) }, "show private data" },
    { "private",               OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(show_private_data) }, "same as show_private_data" },
    { "analyze_frames",        OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, {  .off       = OFFSET(do_analyze_frames) }, "analyze frames to provide additional stream-level information" },
    { "bitexact",              OPT_TYPE_BOOL,        OPT_EXPERT | OPT_FLAG_OFFSET, { .off       = OFFSET(do_bitexact)}, "force bitexact output" },
...
static inline int check_section_show_entries(FFProbeGlobalParam*gp,int section_id)
{
    ...
    const AVTextFormatSection *section = &sections[section_id];
    for (const int *id = section->children_ids; *id != -1; id++)
        if (check_section_show_entries(gp,*id))
            return 1;
    return 0;
}

#define SET_DO_SHOW(id, varname,param) do {                                   \
        if (check_section_show_entries(&param, SECTION_ID_##id))                \
            param.do_show_##varname = 1;                                      \
    } while (0)
...
int ffprobe(int argc, const char **argv)
{
    ...
    global_params.show_private_data = 1;
    ...
    global_params.options = real_options;
    parse_loglevel(argc, argv, global_params.options, &global_params);
    avformat_network_init();
#if CONFIG_AVDEVICE
    avdevice_register_all();
#endif

    show_banner(argc, argv, global_params.options, &global_params);
    ret = parse_options(&global_params, argc, argv, global_params.options, opt_input_file);
    if (ret < 0) {
        ret = (ret == AVERROR_EXIT) ? 0 : ret;
        goto end;
    }

    if (global_params.do_show_log)
        av_log_set_callback(log_callback);

    /* mark things to show, based on -show_entries */
    SET_DO_SHOW(CHAPTERS, chapters,global_params);
    SET_DO_SHOW(ERROR, error,global_params);
    SET_DO_SHOW(FORMAT, format,global_params);
    SET_DO_SHOW(FRAMES, frames,global_params);
    SET_DO_SHOW(LIBRARY_VERSIONS, library_versions,global_params);
    SET_DO_SHOW(PACKETS, packets,global_params);
    SET_DO_SHOW(PIXEL_FORMATS, pixel_formats,global_params);
    SET_DO_SHOW(PIXEL_FORMAT_FLAGS, pixel_format_flags,global_params);
    SET_DO_SHOW(PIXEL_FORMAT_COMPONENTS, pixel_format_components,global_params);
    SET_DO_SHOW(PROGRAM_VERSION, program_version,global_params);
    SET_DO_SHOW(PROGRAMS, programs,global_params);
    SET_DO_SHOW(STREAM_GROUP_DISPOSITION, stream_group_disposition,global_params);
    SET_DO_SHOW(STREAM_GROUPS, stream_groups,global_params);
    SET_DO_SHOW(STREAM_GROUP_COMPONENTS, stream_group_components,global_params);
    SET_DO_SHOW(STREAMS, streams,global_params);
    SET_DO_SHOW(STREAM_DISPOSITION, stream_disposition,global_params);
    SET_DO_SHOW(PROGRAM_STREAM_DISPOSITION, stream_disposition,global_params);
    SET_DO_SHOW(STREAM_GROUP_STREAM_DISPOSITION, stream_disposition,global_params);

    SET_DO_SHOW(CHAPTER_TAGS, chapter_tags,global_params);
    SET_DO_SHOW(FORMAT_TAGS, format_tags,global_params);
    SET_DO_SHOW(FRAME_TAGS, frame_tags,global_params);
    SET_DO_SHOW(PROGRAM_TAGS, program_tags,global_params);
    SET_DO_SHOW(STREAM_GROUP_TAGS, stream_group_tags,global_params);
    SET_DO_SHOW(STREAM_TAGS, stream_tags,global_params);
    SET_DO_SHOW(PROGRAM_STREAM_TAGS, stream_tags,global_params);
    SET_DO_SHOW(STREAM_GROUP_STREAM_TAGS, stream_tags,global_params);
    SET_DO_SHOW(PACKET_TAGS, packet_tags,global_params);

    if (global_params.do_bitexact && (global_params.do_show_program_version || global_params.do_show_library_versions)) {
        av_log(NULL, AV_LOG_ERROR,
    ...
    if (!global_params.output_format)
        global_params.output_format = av_strdup("default");
    if (!global_params.output_format) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
    f_name = av_strtok(global_params.output_format, "=", &buf);
    if (!f_name) {
        av_log(NULL, AV_LOG_ERROR,
    ...
    if (global_params.data_dump_format) {
        if (!strcmp(global_params.data_dump_format, "xxd")) {
            data_dump_format_id = AV_TEXTFORMAT_DATADUMP_XXD;
        } else if (!strcmp(global_params.data_dump_format, "base64")) {
            data_dump_format_id = AV_TEXTFORMAT_DATADUMP_BASE64;
        } else {
            av_log(NULL, AV_LOG_ERROR, "Unknown data dump format with name '%s'\n", global_params.data_dump_format);
            ret = AVERROR(EINVAL);
            goto end;
        }
    }

    if (global_params.output_filename) {
        ret = avtextwriter_create_file(&wctx, global_params.output_filename);
    } else
    ...
    AVTextFormatOptions tf_options = {
        ...
        .show_optional_fields = global_params.show_optional_fields,
        .show_value_unit = global_params.show_value_unit,
        .use_value_prefix = global_params.use_value_prefix,
        .use_byte_value_binary_prefix = global_params.use_byte_value_binary_prefix,
        .use_value_sexagesimal_format = global_params.use_value_sexagesimal_format,
        .data_dump_format             = data_dump_format_id,
    };

    if ((ret = avtext_context_open(&tctx, f, wctx, f_args, sections, FF_ARRAY_ELEMS(sections), tf_options, global_params.show_data_hash)) >= 0) {
        tctx->ffprobe_global_params = &global_params;
        ...
        if (global_params.do_show_program_version)
            ffprobe_show_program_version(tctx);
        if (global_params.do_show_library_versions)
            ffprobe_show_library_versions(tctx);
        if (global_params.do_show_pixel_formats)
            ffprobe_show_pixel_formats(tctx);

        if (!global_params.input_filename &&
            ((global_params.do_show_format || global_params.do_show_programs || global_params.do_show_stream_groups || global_params.do_show_streams || global_params.do_show_chapters || global_params.do_show_packets || global_params.do_show_error) ||
             (!global_params.do_show_program_version && !global_params.do_show_library_versions && !global_params.do_show_pixel_formats))) {
            show_usage();
            av_log(NULL, AV_LOG_ERROR, "You have to specify one input file.\n");
            av_log(NULL, AV_LOG_ERROR, "Use -h to get full help or, even better, run 'man %s'.\n", program_name);
            ret = AVERROR(EINVAL);
        } else if (global_params.input_filename) {
            ret = probe_file(tctx, global_params.input_filename, global_params.print_input_filename);
            if (ret < 0 && global_params.do_show_error)
                show_error(tctx, ret);
        }
    ...
    av_freep(&global_params.output_format);
    av_freep(&global_params.output_filename);
    av_freep(&global_params.input_filename);
    av_freep(&global_params.print_input_filename);
    av_freep(&global_params.read_intervals);

    uninit_opts(&global_params);
    for (size_t i = 0; i < FF_ARRAY_ELEMS(global_params.selected_entries); ++i)
        av_dict_free(&global_params.selected_entries[i].entries_to_show);
